...
var Syncer = function(hubic, options) {
	...
	var now = new Date();
	this._options.backupDirectoryName = this._options.backupDirectoryName || "___backup";
	this.backupName = Sprintf.sprintf("%04d-%02d-%02d %02d:%02d", now.getFullYear(), now.getMonth() + 1, now.getDate(),
...
Syncer.prototype._syncHFile = function(lfile, hdir, hlist, callback) {
	...
	this.log("[Sync ] file local='" + lfile.path + "' remote='" + hdir.path + "'");
...
Syncer.prototype.sync = function(ldir, hdir, callback) {
	...
	if (this._debug) {
		this.log("[Sync ] Directory local='" + ldir.path + "' remote='" + hdir.path + "'");
	} else {
	...
	Async.parallel([ function(callback) {
		...
		Async.eachLimit(Object.keys(llist), self._eachLimit, function(name, callback) {
			var lfile = llist[name];
			var hfile = hlist[name];

			self.log("[Sync ] lfile='" + ((lfile) ? lfile.path : null) + "' hfile='" + ((hfile) ? hfile.path : null) + "'");
			...
			if (hfile) {
				...
				if (lfile.isDirectory == hfile.isDirectory) {
					...
					if (hfile.lastModified.getTime() > lfile.lastModified.getTime() && hfile.size == lfile.size) {
						return callback(null);
					}
				}

				self.log("[Sync ] File modified  directory=" + lfile.isDirectory + "/" + hfile.isDirectory + " date=" +
						hfile.lastModified + "/" + lfile.lastModified + " size=" + hfile.size + "/" + lfile.size);
			...
			Async.eachLimit(Object.keys(hlist), self._eachLimit, function(name, callback) {
				var hfile = hlist[name];
				...
				if (self._options.uploadingPrefix && name.indexOf(self._options.uploadingPrefix) === 0) {
