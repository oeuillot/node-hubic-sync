...
Swift.prototype.putSegment = function(containerName, remotePath, localPath,
  ...
  if (this._options.progress) {
    readStream.on('data', function(chunk) {
      if (!self._slots[remotePath]) {
        return;
      }
      self._slots[remotePath].readBytes += chunk.length;
    ...
    readStream.on('end', function(chunk) {
      ...
      self._slots[remotePath].end = Date.now();
...
function showSlots(slots) {
  ...
  for ( var path in slots) {
    var infos = slots[path];
    ...
    s += " [" + Math.floor(100 * infos.readBytes / infos.realSize) + "% "
        + Math.floor((infos.readBytes / 1024) / ((now - infos.start) / 1000))
        + "kb/s]";
  }
  if (!s) {
    s += " []";
