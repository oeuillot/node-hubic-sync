...
Swift.prototype.getFiles = function(containerName, options, callback) {
	...
	this.log("[swift] getFiles: containerName='" + containerName + "' path='" + options.prefix + "'");
	...
	this.logRequest("LIST '" + URL.format(targetURL) + "' prefix='" + options.prefix + "'");
	...
	request({
		...
		self.logRequest("LIST '" + URL.format(targetURL) + "' prefix='" + options.prefix + "' =>> " +
				(res && res.statusCode));
		...
		if (!err) {
			err = new Error("GET Request unsuccessful: path='" + options.prefix + "' statusCode=" + (res && res.statusCode));
		}
		return callback(err);
	});
};
...
Swift.prototype.$delete = function(containerName, path, ignoreError, callback) {
	this.log("[swift] delete: containerName='" + containerName + "' path='" + path + "'");

	var self = this;
	var targetURL = URL.parse(self.tokens.storageUrl + '/' + containerName + '/' + normalizePath(path));
	...
	request({
		...
		self.logRequest("DELETE '" + URL.format(targetURL) + "' => " + (res && res.statusCode) + " '" + body + "'");
		...
		if (!err) {
			err = new Error("DELETE Request unsuccessful: path='" + path + "' statusCode=" + (res && res.statusCode) +
					" body='" + res.body + "'");
		}
		callback(err);
	});
...
Swift.prototype.mkdir = function(containerName, path, callback) {
	this.log("[swift] mkdir: containerName='" + containerName + "' path='" + path + "'");
	...
	request({
		...
		self.logRequest("MKDIR '" + URL.format(targetURL) + "' => " + (res && res.statusCode) + " '" + body + "'");
		...
		if (!err) {
			err = new Error("MKDIR Request unsuccessful: path='" + path + "' statusCode=" + (res && res.statusCode) +
					" body='" + body + "'");
		}
		return callback(err);
	});
...
Swift.prototype.copy = function(containerName, destName, sourceName, callback) {
	this.log("[swift] copy: destName='" + destName + "' sourceName='" + sourceName + "'");

	var self = this;
	var targetURL = URL.parse(self.tokens.storageUrl + '/' + containerName + '/' + normalizePath(destName));
	...
	request({
		...
		self.logRequest("COPY '" + URL.format(targetURL) + "' => " + (res && res.statusCode) + " '" + body + "'");
		...
		if (!err) {
			err = new Error("COPY Request unsuccessful: path='" + sourceName + "' statusCode=" + (res && res.statusCode));
		}
		return callback(err);
	});
};

Swift.prototype.put = function(containerName, remotePath, localPath, size, hlist, uploadQueue, cmdQueue, callback) {
	...
	Async.map(list, function(partId, callback) {
		...
		uploadQueue.unshift(function(callback) {
			self.putSegment(containerName, prefix + rp, localPath, size, partId, callback);
		...
		cmdQueue.unshift(function(callback) {
			...
			self.mergeSegments(containerName, mergedPath, prefix + segmentName + " -", size, function(error) {
				if (error) {
					return callback(error);
				}
...
Swift.prototype.mergeSegments = function(containerName, remotePath, segmentsPath, size, callback) {
	this.log("[swift] mergeSegments: path='" + remotePath + "' segmentsPath='" + segmentsPath + "'");

	var self = this;
	var targetURL = URL.parse(self.tokens.storageUrl + '/' + containerName + '/' + normalizePath(remotePath));

	var objectPath = containerName + '/' + normalizePath(segmentsPath);

	this.logRequest("MERGE '" + URL.format(targetURL) + "' objectPath='" + objectPath + "'");
	if (this._dryRun) {
	...
	var req = request({
		...
		self.logRequest("MERGE '" + URL.format(targetURL) + "' => " + (res && res.statusCode) + " '" + body + "'");
		...
		if (!err) {
			err = new Error("MERGE Request unsuccessful: url='" + remotePath + "' statusCode=" + (res && res.statusCode) +
					" body='" + body + "'");
		}
		return callback(err);
	});
...
Swift.prototype.putSegment = function(containerName, remotePath, localPath, size, partId, callback) {
	this.log("[swift] putSegment: remotePath='" + remotePath + "' localPath='" + localPath + "' size=" + size +
			" partId=" + partId);
	...
	this.logRequest("PUT '" + URL.format(targetURL) + "'  local='" + localPath + "' startOffset=" + streamOptions.start +
			" endOffset=" + streamOptions.end);
	if (this._dryRun) {
	...
	var req = request({
		...
		self.logRequest("PUT '" + URL.format(targetURL) + "' => " + (res && res.statusCode) + " '" + body + "'");

		if (!err && res && res.statusCode >= 200 && res.statusCode < 300) {
			self.logRequest("Time " + dt + "ms " + Math.floor((realSize / 1024) / (dt / 1000)) + "kb/s");

			return callback(null);
		}

		if (!err) {
			err = new Error("PUT Request unsuccessful: path='" + remotePath + "' statusCode=" + (res && res.statusCode) +
					" body='" + body + "'");
		}
		return callback(err);
	});
...
Swift.prototype._showSlots = function() {
	...
	for ( var path in this._slots) {
		...
		if (infos.end) {
			s += " [ACK " + Math.floor((infos.readBytes / 1024) / ((infos.end - infos.start) / 1000)) + "kb/s]";
			continue;
		}
